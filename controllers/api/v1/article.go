...
func GetArticle(c *gin.Context) {
	...
	if ! valid.HasErrors() {
		if models.ExistArticleByID(id) {
			...
			code = exception.ARTICLE_NOT_EXIST
		}
	} else {
...
func AddArticle(c *gin.Context) {
	...
	valid := validation.Validation{}
	valid.Min(tagId, 1, "tag_id").Message("标签ID必须大于0")
	valid.Required(title, "title").Message("标签标题不能为空")
...
func EditArticle(c *gin.Context) {
	...
	valid.Required(modifiedBy, "modified_by").Message("编辑者不能为空")
	valid.MaxSize(modifiedBy, 100, "modified_by").Message("编辑者最长为100字符")
